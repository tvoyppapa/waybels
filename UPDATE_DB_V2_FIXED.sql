-- ===============================================
-- Обновление базы данных WayBels v2.0 (ИСПРАВЛЕННАЯ ВЕРСИЯ)
-- ===============================================

USE wibs;

-- ===============================================
-- 1. Обновление таблицы users (добавляем колонки)
-- ===============================================

ALTER TABLE users ADD COLUMN IF NOT EXISTS username VARCHAR(50) UNIQUE AFTER email;
ALTER TABLE users ADD COLUMN IF NOT EXISTS bio TEXT NULL AFTER avatar;
ALTER TABLE users ADD COLUMN IF NOT EXISTS theme ENUM('light', 'dark') DEFAULT 'light' AFTER bio;
ALTER TABLE users ADD COLUMN IF NOT EXISTS username_changed_at TIMESTAMP NULL AFTER theme;
ALTER TABLE users ADD COLUMN IF NOT EXISTS balance DECIMAL(10,2) DEFAULT 0.00 AFTER username_changed_at;
ALTER TABLE users ADD COLUMN IF NOT EXISTS is_online BOOLEAN DEFAULT FALSE AFTER balance;
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_seen TIMESTAMP NULL AFTER is_online;

-- ===============================================
-- 2. Обновление таблицы messages (добавляем колонки)
-- ===============================================

ALTER TABLE messages ADD COLUMN IF NOT EXISTS status ENUM('sent', 'delivered', 'read') DEFAULT 'sent' AFTER is_read;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS file_url VARCHAR(255) NULL AFTER message;
ALTER TABLE messages ADD COLUMN IF NOT EXISTS file_type VARCHAR(50) NULL AFTER file_url;

-- ===============================================
-- 3. Настройки чата (обои, кастомизация)
-- ===============================================
...
-- ===============================================
-- 4. Календарь доступности репетитора
-- ===============================================
...
-- Добавляем username существующим пользователям
UPDATE users SET username = CONCAT('user_', id) WHERE username IS NULL;

-- Создаем бота Wibs
INSERT IGNORE INTO users (name, email, password, username, role, avatar) VALUES
...
INSERT IGNORE INTO achievements (name, description, icon, type, requirement, reward_points) VALUES
